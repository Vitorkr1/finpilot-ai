...
export function AgendaPage() {
  ...
  const [form, setForm] = useState({ technicianId: '', startTime: '08:00', endTime: '09:00' })
  const [submitting, setSubmitting] = useState(false)

  async function loadAppointments(forDate: string) {
    setLoading(true)
    try {
      const { data } = await api.get<Appointment[]>('/appointments', { params: { date: forDate } })
    ...
    <AppLayout>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {appointments.map((a) => (
            <div key={a._id} className="flex items-center justify-between rounded-xl border border-slate-200 bg-white p-3 text-sm">
              <span className="font-medium text-slate-900">
                {a.startTime}–{a.endTime}
              </span>
              <span className="text-slate-600">{technicianName(a.technicianId)}</span>
